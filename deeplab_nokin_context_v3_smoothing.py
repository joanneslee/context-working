...
class Iterator:
    ...
    def GenerateContext(self, SAVE=False):
        ...
        # print("getGrasperJawData()", GrasperJaws, GrasperFrames)
        ...
        for Trial in FilenamesInTask:
            ...
            # TrialRoot = os.path.join(self.masksDir, Trial)
            contextLines = []
            ctxFName = os.path.join(self.ctxConsensusDir, Trial + ".txt")
            ctxPredFName = os.path.join(self.context_output, Trial + ".txt")
            frameNum = 0
            currentRing = 7
            for root, dirs, files in os.walk(self.masksDir):
                for label in dirs:
                    # masks separated by label_class
                    TrialLabelRoot = os.path.join(self.masksDir, label)
                    # print("TrialLabelRoot: ", TrialLabelRoot)
                    for root, dirs, files in os.walk(TrialLabelRoot):
                        files.sort()
                        for file in files:
                            ...
                            # print("TRIALFNAME: ", trialFname)
                            imageFName = os.path.join(imageRoot, file)
                            cogitoRoot = root.replace("masks", "annotations")
                            ...
                            if "Suturing" in self.task: 
                                ...
                                print("GrasperClosestIndex: ", str(GrasperClosestIndex))
                                print("GRASPERJAWPOINTS: ", GrasperJawPoints)
                            ...
                            # threadRoot = root.replace("masks","deeplab_thread_v3")
                            threadRoot = os.path.join(self.masksDir, "deeplab_thread_v3", trialFname)
                            # print("THREADROOT: ", threadRoot)
                            # ringRoot = root.replace("images","deeplab_rings_v3")
                            # needleRoot = root.replace("images","deeplab_needle_v3")
                            annotationRoot = root.replace("masks", "annotations")
                            ...
                            # print("THREADMASKIMAGE: ", threadMaskImage)
                            ...
                            L_Gripping, R_Gripping, L_Dist, R_Dist, Grasper_DistX = self.processGrasperData(
                                GrasperJawPoints)
                            ...
                            pred = self.GetCommonShapesPred(ThreadContours, LgrasperPoints, RgrasperPoints)
                            ...
                            if ("Knot" in self.task):
                                ...
                                # dev
                                ctxPredLine, LG_inter_T, RG_inter_T = self.GenerateContextLineKT(pred, gt, L_Gripping,
                                                                                                 R_Gripping,
                                                                                                 frameNumber,
                                                                                                 contextLines,
                                                                                                 Grasper_DistX)
                                ...
                                self.DrawSingleImageContextKT(pred, gt, GrasperJawPoints, imageFName, outputFName, CtxI,
                                                              ctxPredLine, frameNumber, L_Gripping, R_Gripping,
                                                              LG_inter_T, RG_inter_T)

                            if ("Needle" in self.task):
                                # pred, gt = self.GetCommonShapes(gtPolygons,gtKeypoints,SingleThreadPoints,ThreadContours,LgrasperPoints,RgrasperPoints)
                                ringShapes, ringShapes_gt = self.GetRingShapes(Ring4Points, Ring5Points, Ring6Points,
                                                                               Ring7Points, gtPolygons)
                                ...
                                ctxPredLine, LG_inter_T, RG_inter_T, messages = self.GenerateContextLineNP(pred, gt,
                                                                                                           ringShapes,
                                                                                                           ringShapes_gt,
                                                                                                           needleShape,
                                                                                                           needleShape_gt,
                                ...
                                contextLines.append(ctxPredLine)
                                print(Trial, frameNumber, ctxPredLine)
                                self.DrawSingleImageContextNP(pred, gt, ringShapes, ringShapes_gt, needleShape,
                                                              needleShape_gt, GrasperJawPoints, imageFName, outputFName,
                                                              CtxI, ctxPredLine, frameNumber, L_Gripping, R_Gripping,
                                                              LG_inter_T, RG_inter_T, messages, GT=False)
                            if ("Suturing" in self.task):
                                # needleShape, needleShape_gt = self.GetNeedleShapes(NeedlePoints,gtPolygons)
                                needleShape = self.GetNeedleShapesPred(NeedlePoints)
                                ...
                                pred_bisector, pred_tissue, Bisector = self.GetTissueDistPred(TissuePoints, needleShape)
                                ...
                                ctxPredLine, LG_inter_T, RG_inter_T, messages = self.GenerateContextLineSPred(pred,
                                                                                                              needleShape,
                                                                                                              L_Gripping,
                                                                                                              R_Gripping,
                                                                                                              frameNumber,
            ...
            print("proc", os.path.basename(TrialLabelRoot), "count:", frameNum)
    ...
    def processGrasperData(self, GrasperJawPoints):
        try:
            ...
            print("distGraspers:", self.distGraspers(GrasperJawPoints))
        except Exception as e:
            L_Dist = 0
            R_Dist = 0
        PARAM_JAW_DIST = 25
        ...
        if (R_Dist < PARAM_JAW_DIST):
            ...
            R_Gripping = False
        # print("GRASPER_DISTX:", Grasper_DistX)
        # Grasper_DistX = 0
        return L_Gripping, R_Gripping, L_Dist, R_Dist, Grasper_DistX

    def GetTissueDistPred(self, TissuePoints, needleShape):
        pred_bisector, pred_tissue = 1000, 1000
        ...
        return pred_bisector, pred_tissue, Bisector
    ...
    def GenerateContextLineSPred(self, pred, needleShape, L_Gripping, R_Gripping, frameNumber,
                                 contextLines, pred_bisector, pred_tissue, Bisector, GT=False):
        def last5thState(s):
            return s.split(" ")[-1]
        ...
        if GT:
            ...
            NeedleDistances_L = min(
                [LG_dl.distance(shape) for shape in needleShape_gt if not isinstance(needleShape_gt, list)])
            NeedleDistances_R = min(
                [RG_dl.distance(shape) for shape in needleShape_gt if not isinstance(needleShape_gt, list)])
        else:
            ...
            try:
                if not isinstance(needleShape, list):
                    LG_dist_N = min(
                        [LG_dl.distance(shape) for shape in needleShape.geoms if not isinstance(needleShape, list)])
                    RG_dist_N = min(
                        [RG_dl.distance(shape) for shape in needleShape.geoms if not isinstance(needleShape, list)])

                    RG_inter_N = max([LG_dl.intersection(shape).area for shape in needleShape.geoms if
                                      not isinstance(needleShape, list)])
                    LG_inter_N = max([RG_dl.intersection(shape).area for shape in needleShape.geoms if
                                      not isinstance(needleShape, list)])
        ...
        return "" + str(frameNumber) + " " + str(L_G_Hold) + " " + str(L_G_Touch) + " " + str(R_G_Hold) + " " + str(
            R_G_Touch) + " " + str(Extra_State), LG_dist_T, RG_dist_T, messages

    def GenerateContextLineS(self, pred, gt, needleShape, needleShape_gt, L_Gripping, R_Gripping, frameNumber,
                             contextLines, gt_bisector, gt_tissue, pred_bisector, pred_tissue, Bisector, GT=False):
        def last5thState(s):
            return s.split(" ")[-1]
    ...
    def GenerateContextLineNP(self, pred, gt, ringShapes, ringShapes_gt, needleShape, needleShape_gt, L_Gripping,
                              R_Gripping, frameNumber, contextLines, Grasper_DistX, currentRing, GT=False):
        ...
        if GT:
            ...
            try:
                RingDistances_L = [
                    min([LG_dl.distance(shape) for shape in R_GROUP.geoms if not isinstance(LG_dl, list)]) for R_GROUP
                    in ringShapes if not isinstance(R_GROUP, list)] if ringShapes != [] else []

                RingDistances_R = [
                    min([RG_dl.distance(shape) for shape in R_GROUP.geoms if not isinstance(RG_dl, list)]) for R_GROUP
                    in ringShapes if not isinstance(R_GROUP, list)] if ringShapes != [] else []
                RingDistances_N = []
                absMinDistN = 10000
                ringID = -1
                ...
                RingXCoors = [np.average([shape.centroid.x for shape in R_GROUP.geoms]) for R_GROUP in
                              ringShapes] if ringShapes != [] else []

                RingDistances_N = [
                    min([needleShape.distance(shape) for shape in R_GROUP.geoms if not isinstance(needleShape, list)])
                    for R_GROUP in ringShapes if not isinstance(R_GROUP, list)] if ringShapes != [] else []
                RingInter_N = [max([needleShape.intersection(shape).area for shape in R_GROUP.geoms if
                                    not isinstance(needleShape, list)]) for R_GROUP in ringShapes if
                               not isinstance(R_GROUP, list)] if ringShapes != [] else []
        ...
        return "" + str(frameNumber) + " " + str(L_G_Hold) + " " + str(L_G_Touch) + " " + str(R_G_Hold) + " " + str(
            R_G_Touch) + " " + str(Extra_State), LG_inter_T, RG_inter_T, messages

    def GenerateContextLineKT(self, pred, gt, L_Gripping, R_Gripping, frameNumber, contextLines, Grasper_DistX,
                              GT=False):
        [LG_dl, RG_dl, T_dl] = pred
        [LG_Group_gt, RG_Group_gt, T_Group_gt] = gt
        if GT:
        ...
        if (len(contextLines) == 0):
            ...
            if last == "0" and Grasper_DistX < 0:
                ...
                Extra_State = 3  # lose
        return "" + str(frameNumber) + " " + str(L_G_Hold) + " " + str(L_G_Touch) + " " + str(R_G_Hold) + " " + str(
            R_G_Touch) + " " + str(Extra_State), LG_inter_T, RG_inter_T

    def DrawSingleImageContextNP(self, pred, gt, ringShapes, ringShapes_gt, needleShape, needleShape_gt,
                                 GrasperJawPoints, imageFName, outputFName, CtxI, ctxPredLine, frameNumber, L_Gripping,
                                 R_Gripping, LG_inter_T, RG_inter_T, messages, GT=False):
        [LG_dl, RG_dl, T_dl] = pred
        [LG_Group_gt, RG_Group_gt, T_Group_gt] = gt
    ...
    def DrawSingleImageContextS(self, pred, gt, needleShape, needleShape_gt, GrasperJawPoints, imageFName, outputFName,
                                CtxI, ctxPredLine, frameNumber, L_Gripping, R_Gripping, LG_inter_T, RG_inter_T,
                                messages, GT=False):
        [LG_dl, RG_dl, T_dl] = pred
        [LG_Group_gt, RG_Group_gt, T_Group_gt] = gt
        image = cv.imread(imageFName)
    ...
    def DrawSingleImageContextKT(self, pred, gt, GrasperJawPoints, imageFName, outputFName, CtxI, ctxPredLine,
                                 frameNumber, L_Gripping, R_Gripping, LG_inter_T, RG_inter_T, GT=False):
        [LG_dl, RG_dl, T_dl] = pred
        [LG_Group_gt, RG_Group_gt, T_Group_gt] = gt
    ...
    def GetKTShapes(self, gtPolygons, gtKeypoints, SingleThreadPoints, ThreadContours, LgrasperPoints, RgrasperPoints):
        RG_Group_gt = geo.MultiPolygon(
            [geo.Polygon([(gtPolyRG[i], gtPolyRG[i + 1]) for i in range(0, len(gtPolyRG), 2)]) for gtPolyRG in
             gtPolygons["Right Grasper"]]) if "Right Grasper" in gtPolygons.keys() else []
        LG_Group_gt = geo.MultiPolygon(
            [geo.Polygon([(gtPolyLG[i], gtPolyLG[i + 1]) for i in range(0, len(gtPolyLG), 2)]) for gtPolyLG in
    ...
    def GetRingShapes(self, Ring4Points, Ring5Points, Ring6Points, Ring7Points, gtPolygons):
        # print(Ring4Points)
        R4_Group = geo.MultiPolygon([geo.Polygon(ring) for ring in Ring4Points if len(ring) > 3]) if len(
            Ring4Points) > 0 else []
        R5_Group = geo.MultiPolygon([geo.Polygon(ring) for ring in Ring5Points if len(ring) > 3]) if len(
            Ring5Points) > 0 else []
        R6_Group = geo.MultiPolygon([geo.Polygon(ring) for ring in Ring6Points if len(ring) > 3]) if len(
            Ring6Points) > 0 else []
        R7_Group = geo.MultiPolygon([geo.Polygon(ring) for ring in Ring7Points if len(ring) > 3]) if len(
            Ring7Points) > 0 else []

        R4_Group_gt = geo.MultiPolygon(
            [geo.Polygon([(poly[i], poly[i + 1]) for i in range(0, len(poly), 2)]) for poly in
             gtPolygons["Ring_4"]]) if "Ring_4" in gtPolygons.keys() else []
        R5_Group_gt = geo.MultiPolygon(
            [geo.Polygon([(poly[i], poly[i + 1]) for i in range(0, len(poly), 2)]) for poly in
             gtPolygons["Ring_5"]]) if "Ring_5" in gtPolygons.keys() else []
        R6_Group_gt = geo.MultiPolygon(
            [geo.Polygon([(poly[i], poly[i + 1]) for i in range(0, len(poly), 2)]) for poly in
             gtPolygons["Ring_6"]]) if "Ring_6" in gtPolygons.keys() else []
        R7_Group_gt = geo.MultiPolygon(
            [geo.Polygon([(poly[i], poly[i + 1]) for i in range(0, len(poly), 2)]) for poly in
             gtPolygons["Ring_7"]]) if "Ring_7" in gtPolygons.keys() else []

        return [R4_Group, R5_Group, R6_Group, R7_Group], [R4_Group_gt, R5_Group_gt, R6_Group_gt, R7_Group_gt]

    def GetCommonShapesPred(self, ThreadContours, LgrasperPoints, RgrasperPoints):
        '''
        RG_Group_gt = geo.MultiPolygon(
            [geo.Polygon([(poly[i], poly[i + 1]) for i in range(0, len(poly), 2)]) for poly in
             gtPolygons["Right Grasper"]]) if "Right Grasper" in gtPolygons.keys() else []
    ...
    def GetNeedleShapesPred(self, NeedlePoints):

        N_dl = geo.MultiPolygon([geo.Polygon(poly) for poly in NeedlePoints if len(poly) > 3]) if len(
            NeedlePoints) > 0 else []
        # print(len(NeedlePoints))
        '''
        Ring_Group_gt = geo.MultiPolygon(
            [geo.Polygon([(poly[i], poly[i + 1]) for i in range(0, len(poly), 2)]) for poly in
             gtPolygons["Needle Mask"]]) if "Needle Mask" in gtPolygons.keys() else []
        '''
